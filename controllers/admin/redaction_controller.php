...
if(!empty($_POST) && isset($_POST['redaction'])){
if(isset(
    $_POST['titre'], 
    $_POST['description'], 
    $_POST['contenu'], 
    ...
    if(!empty($_POST['titre']) 
        ...
        // if($mode_edition == 0){
            ...
            global $db;
            debug($db);
            $insert = $db->prepare('INSERT INTO `articles` (`title`,`description`,  `content`, `date`, `id_admin`, `id_category`, `id_image` )
            VALUE(?,?,?,NOW(),2,?,?)');
    
            $insert->execute([$article_titre, $article_description, $article_contenu, $article_categorie, $article_image]);
            var_dump($insert);
        // }
        // else{
        ...
        // }
         return $info =  'votre article a bien été posté!';
    }else{
        return $info = 'Veuillez remplir tous les champs !';
    }
}
}
